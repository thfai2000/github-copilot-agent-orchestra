...
The cluster-backed Playwright helpers provision an in-cluster LDAP fixture for auth-provider flows. The current fixture account is `Test User` with password `secret`. The helper now waits until the OAO-API pod can open a TCP connection to the LDAP fixture before the auth-provider tests continue, which avoids startup races on local Kubernetes. When Playwright targets `http://oao.local`, global setup reuses `scripts/ensure-local-oao-access.sh` instead of creating a separate bridge path, and global teardown leaves that shared local bridge running. The shared bridge script keeps its generated PID files, logs, and Nginx config in `.oao-local/`, and that directory is intended to stay untracked. The shared bridge script now also keeps a background monitor alive so dropped UI or API port-forwards are recreated during longer headed runs. Global setup snapshots the current `admin@oao.local` auth state before specs reset it, and global teardown restores that state so local E2E runs no longer leave the superadmin password changed. The Playwright UI helpers also dismiss blocking PrimeVue toast overlays and resolve tab panels through each tab's `aria-controls` target, which keeps workflow-detail assertions aligned with the active tab during longer local runs.
